...
Bạn tự kiểm soát từng scene, từng template, từng brand.
...
npm run pipeline -- output/<slug>/script.json --brand=demo  # ghi đè metadata.brand
```
...
`metadata.brand` (mặc định `"default"`) chọn logo + tên thương hiệu từ
[`brands/`](brands/README.md) — đổi brand thì logo/tên/tagline/URL đổi theo trên toàn bộ
video, không cần sửa từng scene.
...
Muốn dùng [Hermes Agent](https://github.com/) (lệnh `hermes`) thay vì Claude Code? Repo có
sẵn 2 skill bundle trong [`hermes-skills/`](hermes-skills/), tương đương
`/create-template-video` — 1 skill sinh `script.json` từ URL/văn bản, skill kia chạy
pipeline render:
...
Sau đó chỉ cần nói chuyện với `hermes`:
...
Hướng dẫn cài đặt đầy đủ, cách 2 skill phối hợp, và cách đồng bộ lại khi Hermes tự tinh
chỉnh bản cài đặt của nó từ thực tế sử dụng: [`hermes-skills/README.md`](hermes-skills/README.md).
...
Logo và nhận diện thương hiệu (tên, tagline, URL, wordmark ở footer) **không hardcode** trong
template — mọi scene cần hiển thị logo/brand đều lấy dữ liệu từ `brands/<id>/brand.json`, chọn
qua `metadata.brand` (hoặc `--brand=<id>` khi chạy CLI). Đổi brand thì toàn bộ các chỗ đó cập
nhật theo trên cả video; từng scene vẫn có thể tự override riêng 1 field qua `inputs`.

```
brands/
  <id>/
    brand.json   # name, tagline, url, label, logo (đường dẫn tương đối trong thư mục này)
    logo.svg     # hoặc .png/.jpg — được nhúng thành data: URI base64 lúc render
```

| Brand     | Nhận diện                                                          |
| --------- | --------------------------------------------------------------------- |
| `default` | Dùng khi `metadata.brand` bị bỏ trống — sửa `brands/default/brand.json` thành nhận diện của bạn |
| `demo`    | Brand mẫu, giữ cố định làm fixture tham khảo/test                     |

Tài liệu đầy đủ + cách thêm brand mới: [`brands/README.md`](brands/README.md).

> Logo được nhúng dạng base64 `data:` URI thay vì đường dẫn `file://` — Chromium (qua
> HyperFrames) chặn tải resource `file://` nằm ngoài thư mục riêng của template, nên đường dẫn
> logo khác thư mục sẽ âm thầm không hiển thị được.
